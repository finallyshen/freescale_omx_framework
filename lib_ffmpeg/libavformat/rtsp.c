...
#define READ_PACKET_TIMEOUT_S 2
...
static int get_sockaddr(const char *buf, struct sockaddr_storage *sock)
{
    ...
    if (getaddrinfo(buf, NULL, &hints, &ai))
        return -1;
    memcpy(sock, ai->ai_addr, FFMIN(sizeof(*sock), (unsigned int)ai->ai_addrlen));
    freeaddrinfo(ai);
    return 0;
}
...
int ff_sdp_parse(AVFormatContext *s, const char *content)
{
    ...
    for (;;) {
        ...
        q = buf;
        while (*p != '\n' && *p != '\r' && *p != '\0') {
            if ((unsigned int)(q - buf) < sizeof(buf) - 1)
                *q++ = *p;
            p++;
        }
        *q = '\0';
...
void ff_rtsp_skip_packet(AVFormatContext *s)
{
    ...
    while (len > 0) {
        len1 = len;
        if ((unsigned int)len1 > sizeof(buf))
            len1 = sizeof(buf);
                       ...
                       int return_on_interleaved_data, const char *method)
{
    ...
    for (;;) {
        ...
        *q = '\0';
        ...
        av_dlog(s, "line='%s'\n", buf);
        av_log(NULL, AV_LOG_INFO, "line='%s'\n", buf);
                                               ...
                                               int send_content_length)
{
    ...
    if (rt->control_transport == RTSP_MODE_TUNNEL) {
        av_base64_encode(base64buf, sizeof(base64buf), (uint8_t*)buf, strlen(buf));
        out_buf = base64buf;
    }
    ...
    ffurl_write(rt->rtsp_hd_out, (unsigned char*)out_buf, strlen(out_buf));
    if (send_content_length > 0 && send_content) {
        if (rt->control_transport == RTSP_MODE_TUNNEL) {
...
retry:
    ...
    if(content_ptr)
        av_log(NULL, AV_LOG_INFO, "%s return: %s\n", method, *content_ptr);
                              ...
                              int lower_transport, const char *real_challenge)
{
    ...
    for (j = RTSP_RTP_PORT_MIN, i = 0; i < rt->nb_rtsp_streams; ++i) {
        ...
        case RTSP_LOWER_TRANSPORT_UDP_MULTICAST: {
            ...
            getnameinfo((struct sockaddr*) &addr, sizeof(struct sockaddr),
...
int ff_rtsp_connect(AVFormatContext *s)
{
    ...
    memset(&reply1, 0, sizeof(RTSPMessageHeader));    
...
static int sdp_probe(AVProbeData *p1)
{
    ...
    while (p < p_end && *p != '\0') {
        if (p + sizeof("c=IN IP") - 1 < p_end &&
            av_strstart((char *)p, "c=IN IP", NULL))
...
static int sdp_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    size = avio_read(s->pb, (unsigned char*)content, SDP_MAX_SIZE - 1);
    if (size <= 0) {
        av_free(content);
                           ...
                           AVFormatParameters *ap)
{
    ...
    ffio_init_context(&pb, (unsigned char*)sdp, strlen(sdp), 0, NULL, NULL, NULL, NULL);
    s->pb = &pb;
