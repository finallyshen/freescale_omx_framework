...
static int asf_read_language_list(AVFormatContext *s, int64_t size)
{
	...
	for(j = 0; j < stream_count; j++)
	{
		...
		if ((unsigned int)(ret = avio_get_str16le(pb, lang_len, lang, sizeof(lang))) < lang_len)
...
static int asf_read_marker(AVFormatContext *s, int64_t size)
{
	...
	for(i=0; i<count; i++)
	{
		...
		if ((ret = avio_get_str16le(pb, name_len * 2, name, sizeof(name))) < name_len)
			avio_skip(pb, name_len - ret);
		ff_new_chapter(s, i, (AVRational)
		{
			1, 10000000
		}, pres_time, AV_NOPTS_VALUE, name );
	}
...
static int asf_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
	...
	for(;;)
	{
		...
		if(avio_tell(pb) != (int64_t)gpos + gsize)
...
//            av_log(s, AV_LOG_DEBUG, "\n");
	...
	if (asf->packet_flags & 0x01)
	{
		...
		if((int)asf->packet_frag_size > asf->packet_size_left - rsize)
		{
...
static int ff_asf_parse_packet(AVFormatContext *s, AVIOContext *pb, AVPacket *pkt)
{
	...
	for (;;)
	{
		...
		if(   /*asf->packet_frag_size == asf->packet_obj_size*/
		    asf_st->frag_offset + asf->packet_frag_size <= (unsigned int)asf_st->pkt.size
		    && asf_st->frag_offset + asf->packet_frag_size > (unsigned int)asf->packet_obj_size)
		{
		...
		if (   asf_st->pkt.size != asf->packet_obj_size
		        || asf_st->frag_offset + asf->packet_frag_size > (unsigned int)asf_st->pkt.size)   //FIXME is this condition sufficient?
		{
			if(asf_st->pkt.data)
			{
...
//s->streams[asf->stream_index]->codec->codec_type == AVMEDIA_TYPE_AUDIO, asf->packet_obj_size);
		...
		if(   asf->packet_frag_offset >= (unsigned int)asf_st->pkt.size
		...
		ret = avio_read(pb, asf_st->pkt.data + asf->packet_frag_offset,
		                asf->packet_frag_size);
		if (ret != (int)asf->packet_frag_size)
		{
