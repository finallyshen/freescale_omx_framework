                             ...
                             const uint8_t *buf, const uint8_t *end)
{
    ...
    while (end - p >= 2) {
        ...
        if (item_len < 2 || (unsigned int)(end - p) < item_len || config_item > 4)
                                ...
                                const uint8_t *buf, const uint8_t *end)
{
    ...
    if (end - p < (int)len + (type == 0x7F) || id >= 0x80)
        return AVERROR_INVALIDDATA;
    if (type == 0x7F)
        type |= *p++ << 8;

    /* copy data into a temporary buffer */
    to_copy = FFMIN(len + (p - &buf[1]), (unsigned int)(0x800 - qdm->len[id]));
                             ...
                             const uint8_t *buf, int len, int flags)
{
    ...
    if (len > 0) {
        ...
        if (++qdm->n_pkts < (unsigned int)qdm->subpkts_per_block)
