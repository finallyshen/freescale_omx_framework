...
static int sap_write_header(AVFormatContext *s)
{
    ...
    av_strlcpy((char *)&sap->ann[pos], "application/sdp", sap->ann_size - pos);
    pos += strlen((char *)&sap->ann[pos]) + 1;

    if (avf_sdp_create(contexts, s->nb_streams, (char *)&sap->ann[pos],
                       sap->ann_size - pos)) {
