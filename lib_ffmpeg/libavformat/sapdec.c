...
#define MIME "application/sdp"
        if (strcmp((char *)&recvbuf[pos], MIME) == 0) {
            pos += strlen(MIME) + 1;
        } else if (strncmp((char *)&recvbuf[pos], "v=0\r\n", 5) == 0) {
        ...
        sap->sdp = av_strdup((const char *)&recvbuf[pos]);
        break;
    }

    av_log(s, AV_LOG_VERBOSE, "SDP:\n%s\n", sap->sdp);
    ffio_init_context(&sap->sdp_pb, (unsigned char *)sap->sdp, strlen(sap->sdp), 0, NULL, NULL,
                  NULL, NULL);
    ...
    for (i = 0; i < (int)sap->sdp_ctx->nb_streams; i++) {
