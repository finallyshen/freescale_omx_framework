...
static int set_codec_from_probe_data(AVFormatContext *s, AVStream *st, AVProbeData *pd)
{
    static const struct {
        ...
        { 0, CODEC_ID_NONE, AVMEDIA_TYPE_UNKNOWN}
    };
    int score;
...
void ff_reduce_index(AVFormatContext *s, int stream_index)
{
    ...
    if((unsigned)st->nb_index_entries >= max_entries){
        int i;
        for(i=0; 2*i<(int)st->nb_index_entries; i++)
...
int av_add_index_entry(AVStream *st,
                       int64_t pos, int64_t timestamp, int size, int distance, int flags)
{
    return ff_add_index_entry(&st->index_entries, (int*)&st->nb_index_entries,
...
int av_seek_frame_binary(AVFormatContext *s, int stream_index, int64_t target_ts, int flags){
    ...
    int64_t pos_min = 0LL, pos_max = 0LL, pos, pos_limit;
    ...
    if(st->index_entries){
        ...
        assert((unsigned int)index < st->nb_index_entries);
        if(index >= 0){
            e= &st->index_entries[index];
                                 ...
                                 int stream_index, int64_t timestamp, int flags)
{
    ...
    if(index < 0 || (unsigned int)index==st->nb_index_entries-1){
        int i;
        AVPacket pkt;
...
//                    av_log(NULL, AV_LOG_ERROR, "%f\n", dur);
                ...
                for (i=1; i<(int)FF_ARRAY_ELEMS(st->info->duration_error); i++) {
        ...
        if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (st->info->duration_count && !st->r_frame_rate.num
               ...
               st->time_base.num*duration_sum[i]/st->info->duration_count*101LL > st->time_base.den*/){
                ...
                for (j=1; j<(int)FF_ARRAY_ELEMS(st->info->duration_error); j++) {
...
static AVProgram *find_program_from_stream(AVFormatContext *ic, int s)
{
    ...
    for (i = 0; i < (int)ic->nb_programs; i++)
        for (j = 0; j < (int)ic->programs[i]->nb_stream_indexes; j++)
            if ((int)ic->programs[i]->stream_index[j] == s)
                        ...
                        int flags)
{
    ...
    for (i = 0; i < nb_streams; i++) {
        unsigned int real_stream_index = program ? program[i] : (unsigned int)i;
        AVStream *st = ic->streams[real_stream_index];
        AVCodecContext *avctx = st->codec;
        if (avctx->codec_type != type)
            continue;
        if (wanted_stream_nb >= 0 && real_stream_index != (unsigned int)wanted_stream_nb)
            continue;
...
AVProgram *av_new_program(AVFormatContext *ac, int id)
{
    ...
    for(i=0; i<(int)ac->nb_programs; i++)
...
void ff_program_add_stream_index(AVFormatContext *ac, int progid, unsigned int idx)
{
    ...
    for(i=0; i<(int)ac->nb_programs; i++){
        if(ac->programs[i]->id != progid)
            continue;
        program = ac->programs[i];
        for(j=0; j<(int)program->nb_stream_indexes; j++)
                    ...
                    int is_output)
{
    ...
    for (i = 0; i < (int)ic->nb_chapters; i++) {
    ...
    if(ic->nb_programs) {
        ...
        for(j=0; j<(int)ic->nb_programs; j++) {
            AVMetadataTag *name = av_metadata_get(ic->programs[j]->metadata,
                                                  "name", NULL, 0);
            av_log(NULL, AV_LOG_INFO, "  Program %d %s\n", ic->programs[j]->id,
                   name ? name->value : "");
            dump_metadata(NULL, ic->programs[j]->metadata, "    ");
            for(k=0; k<(int)ic->programs[j]->nb_stream_indexes; k++) {
