...
static int mov_read_dref(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    for (i = 0; i < (int)sc->drefs_count; i++) {
...
static int mov_read_ftyp(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    av_metadata_set2(&c->fc->metadata, "major_brand", (char *)type, 0);
        ...
        if(st->codec->codec_type==AVMEDIA_TYPE_VIDEO) {
            ...
            if ((color_depth == 2) || (color_depth == 4) ||
                ...
                if (color_greyscale) {
                    ...
                    if ((color_start <= 255) &&
                        (color_end <= 255)) {
                        for (j = color_start; j <= (int)color_end; j++) {
...
static int mov_read_stsz(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if ((unsigned int)avio_read(pb, buf, num_bytes) < num_bytes) {
        av_freep(&sc->sample_sizes);
        av_free(buf);
        return -1;
    }
...
static void mov_build_index(MOVContext *mov, AVStream *st)
{
    ...
    if (!(st->codec->codec_type == AVMEDIA_TYPE_AUDIO &&
        ...
        for (i = 0; i < (int)sc->chunk_count; i++) {
...
static int mov_open_dref(AVIOContext **pb, char *src, MOVDref *ref)
{
    ...
    if (ref->nlvl_to > 0 && ref->nlvl_from > 0) {
        ...
        if (i == ref->nlvl_to - 1 && (unsigned int)(src_path - src)  < sizeof(filename)) {
...
static int mov_read_trak(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (sc->dref_id-1 < (int)sc->drefs_count && sc->drefs[sc->dref_id-1].path) {
...
static int mov_read_trun(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    for (i = 0; i < (int)c->fc->nb_streams; i++) {
        if ((unsigned int)c->fc->streams[i]->id == frag->track_id) {
            st = c->fc->streams[i];
            break;
        }
    }
    ...
    sc = st->priv_data;
    if ((unsigned int)sc->pseudo_stream_id+1 != frag->stsd_id)
        return 0;
    avio_r8(pb); /* version */
    flags = avio_rb24(pb);
    entries = avio_rb32(pb);
...
static int mov_read_elst(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if((uint64_t)edit_count*12+8 > (uint64_t)atom.size)
        return -1;
...
static void mov_read_chapters(AVFormatContext *s)
{
    ...
    for (i = 0; i < (int)st->nb_index_entries; i++) {
        AVIndexEntry *sample = &st->index_entries[i];
        int64_t end = i+1 < (int)st->nb_index_entries ? st->index_entries[i+1].timestamp : st->duration;
        uint8_t *title;
        uint16_t ch;
        int len, title_len;
        ...
        if (ch == 0xfeff)
            ...
            AV_WB16(title, ch);
            get_strz(sc->pb, (char *)title + 2, len - 1);
        }

        ff_new_chapter(s, i, st->time_base, sample->timestamp, end, (char *)title);
        av_freep(&title);
    }
finish:
...
static AVIndexEntry *mov_find_next_sample(AVFormatContext *s, AVStream **st)
{
    ...
    for (i = 0; i < (int)s->nb_streams; i++) {
        ...
        if (msc->pb && (unsigned int)msc->current_sample < avst->nb_index_entries) {
...
static int mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
 ...
 retry:
    ...
    if (sc->ctts_data) {
        ...
        /* update ctts context */
        sc->ctts_sample++;
        if ((unsigned int)sc->ctts_index < sc->ctts_count &&
            sc->ctts_data[sc->ctts_index].count == (unsigned int)sc->ctts_sample) {
            sc->ctts_index++;
            sc->ctts_sample = 0;
        }
        if (sc->wrong_dts)
        ...
        int64_t next_dts = ((unsigned int)sc->current_sample < st->nb_index_entries) ?
...
static int mov_read_seek(AVFormatContext *s, int stream_index, int64_t sample_time, int flags)
{
    ...
    if ((unsigned int)stream_index >= s->nb_streams)
        return -1;
    if (sample_time < 0)
        sample_time = 0;
...
static int mov_read_close(AVFormatContext *s)
{
    ...
    for (i = 0; i < (int)s->nb_streams; i++) {
        ...
        for (j = 0; j < (int)sc->drefs_count; j++) {
    ...
    if (mov->dv_demux) {
        for(i = 0; i < (int)mov->dv_fctx->nb_streams; i++) {
