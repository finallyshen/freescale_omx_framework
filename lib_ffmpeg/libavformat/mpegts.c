...
static int discard_pid(MpegTSContext *ts, unsigned int pid)
{
    ...
    for(i=0; i<(int)ts->nb_prg; i++) {
        p = &ts->prg[i];
        for(j=0; j<(int)p->nb_pids; j++) {
            if(p->pids[j] != pid)
                continue;
            //is program with id p->id set to be discarded?
            for(k=0; k<(int)ts->stream->nb_programs; k++) {
                if((unsigned int)ts->stream->programs[k]->id == p->id) {
...
static const StreamType ISO_types[] = {
    ...
    { 0,       AVMEDIA_TYPE_UNKNOWN, CODEC_ID_NONE},
};
                         ...
                         int *dec_config_descr_size)
{
    ...
    ffio_init_context(&pb, (unsigned char *)buf, size, 0, NULL, NULL, NULL, NULL);
