...
typedef int (*read_packet_t)(void *opaque, uint8_t *buf, int buf_size);
typedef int (*write_packet_t)(void *opaque, uint8_t *buf, int buf_size);
typedef int64_t (*seek_t)(void *opaque, int64_t offset, int whence);
...
int ffio_fdopen(AVIOContext **s, URLContext *h)
{
    ...
    *s = av_mallocz(sizeof(AVIOContext));
    ...
    if (ffio_init_context(*s, buffer, buffer_size,
                      ...
                      (read_packet_t)ffurl_read, (write_packet_t)ffurl_write, (seek_t)ffurl_seek) < 0) {
        av_free(buffer);
        av_freep(s);
        return AVERROR(EIO);
    }
...
typedef struct DynBuffer {
    unsigned int pos, size, allocated_size;
    uint8_t *buffer;
    int io_buffer_size;
    uint8_t io_buffer[1];
} DynBuffer;
