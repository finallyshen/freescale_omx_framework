...
static int rm_read_extradata(AVIOContext *pb, AVCodecContext *avctx, unsigned size)
{
	...
	if ((unsigned int)avctx->extradata_size != size)
		return AVERROR(EIO);
	return 0;
}
...
static void rm_read_metadata(AVFormatContext *s, int wide)
{
	...
	{
		unsigned int len = wide ? avio_rb16(s->pb) : (unsigned int)avio_r8(s->pb);
                                     ...
                                     AVStream *st, RMStream *ast, int read_all)
{
	...
	else
	{
		...
		case CODEC_ID_RA_288:
			...
			if((unsigned int)ast->audio_framesize >= UINT_MAX / sub_packet_h)
			{
		...
		case CODEC_ID_SIPR:
			...
			if(codecdata_length + FF_INPUT_BUFFER_PADDING_SIZE <= codecdata_length)
			{
...
//        av_log(s, AV_LOG_DEBUG, "fps= %d fps2= %d\n", fps, fps2);
		...
		case 3:
			...
			break;
		case 4:
			st->codec->codec_id = CODEC_ID_RV40;
			break;
		default:
...
static int rm_read_index(AVFormatContext *s)
{
	...
	do
	{
		...
		for (n = 0; n < s->nb_streams; n++)
			if ((unsigned int)s->streams[n]->id == str_id)
			{
				st = s->streams[n];
				break;
			}
...
static int rm_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
	...
	tag = avio_rl32(pb);
	if (tag == (unsigned int)MKTAG('.', 'r', 'a', 0xfd))
	{
		/* very old .ra format */
...
{
	int hdr, seq = 0, pic_num = 0, len2 = 0, pos;
	int type;
                      ...
                      AVStream *st, RMStream *ast, AVPacket *pkt)
{
	...
	rm->audio_pkt_cnt--;
	if ((pkt->pts=pkt->pts = ast->audiotimestamp) != (int64_t)AV_NOPTS_VALUE)
	{
