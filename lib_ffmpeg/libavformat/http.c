...
{"chunksize", "use chunked transfer-encoding for posts, -1 disables it, 0 enables it", OFFSET(chunksize), FF_OPT_TYPE_INT64, 0, -1, 0, 0, 0 }, /* Default to 0, for chunked POSTs */
{NULL, NULL, 0, FF_OPT_TYPE_FLAGS, 0, 0, 0, 0, 0}
};
static const AVClass httpcontext_class = {
    "HTTP", av_default_item_name, options, LIBAVUTIL_VERSION_INT, 0, 0
};

static int64_t http_connect(URLContext *h, const char *path, const char *hoststr,
...
void ff_http_set_headers(URLContext *h, const char *headers)
{
    ...
    av_strlcpy((char *)s->headers, headers, sizeof(s->headers));
    //av_log(NULL, AV_LOG_INFO, "http header: %s\n", s->headers);
}
...
static int http_open(URLContext *h, const char *uri, int flags)
{
    ...
    //av_log(NULL, AV_LOG_INFO, "Connect to: %s\n", s->location);
                        ...
                        const char *auth, int *new_location)
{
    ...
    if (!has_header((char *)s->headers, "\r\nUser-Agent: "))
       len += av_strlcatf(headers + len, sizeof(headers) - len,
                          "User-Agent: %s\r\n", "Android;Linux;iPad;iPhone");
    ...
    av_strlcpy(headers+len, (char *)s->headers, sizeof(headers)-len);
    ...
    if (ffurl_write(s->hd, s->buffer, strlen((char *)s->buffer)) < 0)
        return AVERROR(EIO);
    ...
    for(;;) {
        ...
        //av_log(NULL, AV_LOG_INFO, "header='%s'\n", line);
...
static int64_t http_seek(URLContext *h, int64_t off, int whence)
{
    ...
    if (http_open_cnx(h) < 0) {
        av_log(NULL, AV_LOG_ERROR, "seek http failed.");
        return -1;
    }
