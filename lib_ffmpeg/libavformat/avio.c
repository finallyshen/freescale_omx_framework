...
{
    URLProtocol **p;
    if ((unsigned int)size < sizeof(URLProtocol)) {
...
typedef int (*transfer_func_t)(URLContext *h, unsigned char *buf, int size);
                                        ...
                                        transfer_func_t transfer_func)
{
    ...
    while (len < size_min) {
        ...
        if (len < size && url_interrupt_cb())
            return AVERROR_EXIT;
        if (retries <= 0) {
            av_log(NULL, AV_LOG_INFO, "transfer_func data timeout.\n");
            return len;
        }
...
int ffurl_read(URLContext *h, unsigned char *buf, int size)
{
    if (h->flags & AVIO_WRONLY)
        return AVERROR(EIO);
    return retry_transfer_wrapper(h, buf, size, 1, (transfer_func_t)h->prot->url_read);
}
...
int ffurl_read_complete(URLContext *h, unsigned char *buf, int size)
{
    if (h->flags & AVIO_WRONLY)
        return AVERROR(EIO);
    return retry_transfer_wrapper(h, buf, size, size, (transfer_func_t)h->prot->url_read);
}
...
int ffurl_write(URLContext *h, const unsigned char *buf, int size)
{
    ...
    return retry_transfer_wrapper(h, (unsigned char *)buf, size, size, (transfer_func_t)h->prot->url_write);
}
