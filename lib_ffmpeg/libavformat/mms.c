...
int ff_mms_asf_header_parser(MMSContext *mms)
{
    ...
    if ((unsigned int)mms->asf_header_size < sizeof(ff_asf_guid) * 2 + 22 ||
    ...
    while(end - p >= (int)sizeof(ff_asf_guid) + 8) {
        uint64_t chunksize;
        ...
        if (!chunksize || (int64_t)chunksize > end - p) {
            av_log(NULL, AV_LOG_ERROR,
        ...
        if (!memcmp(p, ff_asf_file_header, sizeof(ff_asf_guid))) {
            /* read packet size */
            if (end - p > (int)sizeof(ff_asf_guid) * 2 + 68) {
                mms->asf_packet_len = AV_RL32(p + sizeof(ff_asf_guid) * 2 + 64);
                if (mms->asf_packet_len <= 0 || mms->asf_packet_len > (int)sizeof(mms->in_buffer)) {
                    av_log(NULL, AV_LOG_ERROR,
            ...
            if (mms->stream_num < MMS_MAX_STREAMS &&
                    (unsigned int)(46 + mms->stream_num * 6) < sizeof(mms->out_buffer)) {
            ...
            if (end - p >= 88) {
                ...
                while (stream_count--) {
                    if ((uint64_t)(end - p) < skip_bytes + 4) {
                        av_log(NULL, AV_LOG_ERROR,
                ...
                while (ext_len_count--) {
                    if ((uint64_t)(end - p) < skip_bytes + 22) {
                        av_log(NULL, AV_LOG_ERROR,
